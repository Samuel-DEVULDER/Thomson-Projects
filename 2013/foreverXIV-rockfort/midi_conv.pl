...
for $curr (@ARGV) {
	if(-e $curr) {$file = $curr;}
	if("-h" eq $curr) {&usage;}
	if("-x" eq $prev) {for(split(/,/,$curr)) {$excl_instr{$_}=1;}}
	if("-t" eq $prev) {
		my($i, @t) = (0, split(/,/,$curr));
		$glb_order{-1} = -1;
...
print "excl: ",join(' ', keys %excl_instr),"\n";
...
sub make_mono {
	...
	for($i=0; $i<=$#zik;) {
		...
		for $key (keys %note) {
			$note{$key} = int($note{$key}*(.8**(($next-$time)/$glb_ticks_per_note)));
		}
...
sub convert_track {
	...
	while($i+1<=$len) {
		...
		# nouveau tempo?
		if($#bpm>=0 && $bpm[0]<=$time1) {
			my($bpm) = $glb_bpm{shift(@bpm)};
			my($t) = 1875000/$bpm;
			push(@trk, sprintf("cBPM,\$%02x,\$%02x", $t>>8, $t&255));
		}
...
sub read_track {
	...
	do {
		...
		if($event == 0xf0) {while(&read_byte!=0xf7) {}}
...
sub dump_midi {
	...
	for $t (@_) {
		...
		print "(",$time-$tm,")\n";
		print sprintf("%-6d %2d %3d *%-3d", $time, $trk, $note-1, $vol);
		$tm = $time;
	}
	print "\n";
}
