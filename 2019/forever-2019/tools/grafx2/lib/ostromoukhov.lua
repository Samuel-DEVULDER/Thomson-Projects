...
function OstroDither:new(palette,attenuation,levels)
	local o = {
		...
		linear_levels = thomson.levels.linear,
...
function OstroDither:setLevelsFromPalette()
	...
	local levels = {r={},g={},b={}}
	for i,v in ipairs(self.linear_levels) do
		if false then
	...
	if self._ostro==nil then
		...
		local function process(tab)
			...
			local function add(i)
				i=3*math.floor(i+.5)
				local c0,c1,c2,norm=t[i+1],t[i+2],t[i+3]
				if type(self.attenuation)=='table' then
					c0=c0*self.attenuation[1]
					c1=c1*self.attenuation[2]
					c2=c2*self.attenuation[3]
					norm=(self.attenuation[4] or .9)/(c0+c1+c2)
				else
					norm=self.attenuation/(c0+c1+c2)
				end
...
function OstroDither:getColorIndex(linearPixel)
	...
	if c==nil then
		...
		for i=1,#self.palette do
			local d = self:dist2(self:_linearPalette(i),linearPixel)
...
function OstroDither:ccDither(screen_w,screen_h,getLinearPixel,pset,serpentine,info) -- dither with color clash
	...
	self.getColorIndex = function(self,p)
		return self:dist2(p, self:_linearPalette(c1))<self:dist2(p, self:_linearPalette(c2)) and c1 or c2
	end
	...
	local findC1C2 = function(x,y,xs,err1)
		...
		if not self:ccAcceptCouple(c1,c2)
			...
			local function eval()
				if self:ccAcceptCouple(c1,c2) then
					...
					for i=1,self.clash_size do
						...
						local e = err.r^2 + err.g^2 + err.b^2
						d = d + e*(3*(1+math.cos(2*3141592653*i/self.clash_size))^2+1)
			...
			if dm<MAX and histo:num(1)>self.clash_size/2+1 then
				local z=c2
