...
local function p(r,g,b) return b*256+g*16+r; end
local pal = {
...
local dither=OstroDither:new(pal, .75)
	...
	self:ccDither(thomson.w,math.floor(thomson.h/2),
				  function(x,y)
					...
					return p:map(function(x)
						local z=1.2
						return .9*Color.ONE*math.sin(z*x/(2*Color.ONE))/math.sin(z)
						 -- 1.2*(1-math.exp(-x/Color.ONE/2))*Color.ONE --(1+x/(2*Color.ONE))
					end)
				  end,
				  function(x,y,c)
					if x%8==0 then
