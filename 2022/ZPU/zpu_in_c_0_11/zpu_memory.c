...
uint32_t memoryReadLong(uint32_t address)
{
    ...
    if (address == 0x080A0014
    ||  address == 0x80000100)
    {
        long v=(long)latched_cycle;
        // printf("readlow = %x\n",v);
        return v;
    }
    if (address == 0x080A0018
    ||  address == 0x80000104)
    {
        long v = (long)(latched_cycle>>32);
        // printf("read high = %x\n",v);
        return v;
    }
...
void memoryWriteLong(uint32_t address, uint32_t value)
{
    ...
    ||  address == 0x80000100)
    {
        if((value & 2)!=0) latched_cycle = cycle;
    }
    address = address & MEMORY_MASK;
    if ((address & 0x3) != 0)
    {
