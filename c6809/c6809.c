...
/*-------------------------------------------------------------------
 * Ajoute un commentaire par rapport à l'adressage relatif
 */
void Rel_Annotation(void)
{
	char *s = linebuffer;
	while(*s) ++s;
	if(s+9 < linebuffer+sizeof(linebuffer))
		sprintf(s, " ; $%04X", eval.operand&0xFFFF);
}
...
const struct {
     ...
     { "ANDCC", 0x00, 0x1C,  3, TRUE , Ass_Immediate },
     ...
     { "ORCC" , 0x00, 0x1A,  3, TRUE , Ass_Immediate },
...
int Ass_MACRO(void)
{
    ...
    if (run.pass == MACROPASS)
    {
		...
		current_macro->quiet = !strncmp(line, "QUIET",5); /* sam */
    }
    else
    {
...
int Ass_ENDM(void)
{
    ...
    if (macro.level)
    {
		...
		run.quiet = current_macroarg->quiet ;
        macro.level-- ;
        return NO_ERROR ;
    }
    else
...
int AssembleLine(void)
{
    ...
    if ((done == FALSE) && (run.pass > MACROPASS))
    {
		...
		run.quiet = current_macroarg->quiet = current_macro->quiet;
    }
